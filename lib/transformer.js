...
module.exports = class Transformer {
  ...
  transform(code, { withAST = false }) {
    return withAST ? this.transformWithAST(code) : this.transformWithReg(code)
  }
  ...
  transformWithReg(code) {
    ...
    return code.replace(reg, (...args) => {
      ...
      try {
        const resolvedModulePath = this.resolver(process.cwd(), modulePath)
        const normalizedPath = normalizePath(resolvedModulePath)
